...
| `prerelease_id` | No | empty string | Prerelease identifier. Supply an alphanumeric string directly, or newline-separated file paths to hash. If omitted, the workflow defaults to the run number. |
...
| Name | Description |
| --- | --- |
| `version` | Published semantic version without a leading `v`, for example `1.2.4` or `1.2.4-dev42`. |
